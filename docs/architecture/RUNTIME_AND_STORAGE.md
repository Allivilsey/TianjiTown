...
| `BuffPlayerEffects` | 校验效果定义与生效记录，应用/修复属性和药水效果，管理持久化药水标记、生命值恢复及失败回滚 |
| `BuffExpirationScheduler` | 统一管理刷新版本、到期任务与截止时间，取消旧任务、过滤旧回调并保留调度失败后的重试时间 |

效果操作和调度状态仍只在主线程访问，数据库读写继续经由 `TownRuntime` 执行。新刷新保留已知截止时间；玩家退出时清理托管效果，同时保留待执行的数据库到期任务；停服时取消任务并使未返回的刷新回调失效。购买效果应用失败时仍先完成退款，再通知调用方。
...
`BuffExpirationSchedulerTest` 覆盖到期时间选择、旧回调失效、退出保留任务、调度失败恢复与停服取消；`BuffRuntimeTest` 覆盖异步刷新顺序、重生清理与恢复、应用失败退款，以及离线玩家到期清理失败后的重试。
