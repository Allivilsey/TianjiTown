...
| `/tianjitown money adjust <小镇代码> <带符号金额> [原因]` | 调整该镇公共余额，在小镇数据库内同时写入余额、账本与审计；正数增加、负数减少，不能为零或导致余额不足 | 全部 |
...
例如 `/tianjitown money adjust sky -100 活动费用更正` 只减少该镇数据库内的公共余额，不改变玩家钱包或旧税收账户余额。
