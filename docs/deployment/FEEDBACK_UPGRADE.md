...
玩家申请代码须为 3～9 个英文字母；底层 Residence 名称对象校验范围为 1～12 位，不能据此把玩家表单要求写成 1～12 位。小镇资料编辑只修改简介和规则，不能修改代码；不存在数据库回填或旧开发配置升级流程。

领地地图固定只包含 25 个 20px 区域按钮，底部“选择操作”进入独立页面显示数量、价格、确认、清空及继续选择。客户端各 GUI 缩放和完整测试服流程须按 [测试入口](../../test/README.md) 留存人工验收证据。
