...
本功能最初由 `1.3.0` 交付，数据库目标版本为 `4.0`。本次迁移新增 `active_buffs`、`resource_orders`，并扩展公共账本类型；不会删除既有申请、治理、经济或领地数据。
...
4. 启动后执行 `/townadmin status`，确认 Flyway 到达 `4.0`，税收、账本和扩张仍可使用。
...
修改后执行 `/townadmin reload`。关闭 Buff 商店只停止新购买，已购买效果继续到期；关闭资源商店只停止新订单，已有订单仍可领取或退款。申请、治理、税收、账本和领地功能不受影响。

回滚前先关闭两个商店并完成未领取订单检查、Buff 退款决定及清算对账，然后停服备份。`1.2.0` 不应直接写入已经迁移到 `4.0` 的数据库；需要回退 JAR 时，同时恢复升级前的 SQLite 备份。该次迁移没有改变 QuickShop、XConomy 和 Residence 的结构，但仍应保留同一时间点备份以便整组恢复。
