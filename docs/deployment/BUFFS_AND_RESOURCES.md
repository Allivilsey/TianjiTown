...
玩家商店由镇长或副镇长操作，使用公共资金购买，选择 1～4 周及商品允许的等级；同类再次购买按新选择覆盖有效项，剩余时间不折价。速度/生命最高 V、安全落地/矿工最高 III，夜视/水下呼吸/防火为 I。商品定义在 `buffs.catalog`，改后重启；价格见 [定价说明](PRICING.md)。

管理员 `buff grant` 是付费代购，固定一周，按 `stacking` 而非玩家自选逻辑计算下一档：默认速度/生命逐级提高并在现有到期时间后增加一周，其他默认商品从购买时刻刷新 I 级一周；满级升级会拒绝。管理员操作需确认。完整参数见 [命令手册](../ADMIN_COMMANDS.md)。
...
- 分别测试属性和药水商品的等级、时长、价格与成员覆盖；玩家普通镇员不能购买，访客不获得成员 Buff。
