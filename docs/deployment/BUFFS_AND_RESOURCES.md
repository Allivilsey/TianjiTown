...
4. 启动后执行 `/tianjitown status` 和 `/tianjitown money reconcile`，确认迁移完成且清算账户无短款。
...
- 收费以每周基础价和所选等级为基准，1/2/3/4 周分别原价/九五折/九折/八五折，每次只有一条扣款流水；再次购买同类 Buff 时覆盖旧生效项。
...
`buffs.shop-enabled` 可通过 `/tianjitown reload` 热更新。关闭后只停止新购买，
已购买效果继续到期并正常清理。
