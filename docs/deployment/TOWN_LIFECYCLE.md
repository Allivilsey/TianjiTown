...
适用于 `1.0.0-SNAPSHOT`。批准后立即预留完整 5×5 区域网格；每个区域为 5×5 区块，初始区域自动激活完整 flag，其他区域保持野外行为并禁止圈地，后续付费激活。建镇需两名初始成员确认，后续采用入镇申请制。完整语法见 [管理员命令](../ADMIN_COMMANDS.md)，字段限制见 [功能说明](../FUNCTIONS_AND_GAMEPLAY.md)。
