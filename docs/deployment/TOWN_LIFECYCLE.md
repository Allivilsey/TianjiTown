...
2. 玩家在三步 Dialog 表单中填写小镇名称、小镇代码、简介、规则和两名初始成员；验证保存草稿、默认成员按钮与 5 分钟冷却的重新提醒，再选择当前区块，看到 3×3 三维粒子边界并确认提交。
3. 管理员依次验证 `application list/approve/reject/change <小镇全名>`，并验证管理员主菜单中的审核按钮会随待办状态改变外观。
4. 批准后同时核对 SQLite、镇长成员记录和 Residence；小镇代码只允许 `1..12` 个英文字母并直接按小写生成 Residence 名称，例如 `SKY` 生成 `sky`。建议使用三个字母，不添加任何前后缀，也不接受小镇 UUID 或旧格式。
