...
当前数据库目标版本为 `6.0`，配置 schema 为 `6`。升级会将旧官员迁移为副镇长、每日返还计数合并为周计数，并新增 Jobs 与 GlobalMarketPlus 收入税记录。迁移前必须保留完整 SQLite 与依赖数据备份。
...
5. 启动后确认 `/townadmin status` 显示 config schema `6`、Flyway schema `6.0` 和 `READY`。
