...
| 配置键后缀 | 默认值 | 含义 |
|---|---|---|
| `handbook-cooldown-minutes` | `60` | 手册领取冷却，管理员发放也遵守 |
...
初始领地固定 5×5 区块，扩张地图固定 5×5 单元。治理窗口也不是可配置项：接任 24 小时、活跃窗口 30 天、投票 72 小时、最多 3 名副镇长。不要增加没有代码读取的旧治理配置键。
...
| `stacking` | 固定时长代购的规则：`LEVEL_UP` 升级、`REFRESH` 刷新、`EXTEND` 延长；影响管理员 `buff grant` |

玩家商店按自选周数和等级覆盖同类效果，不按 `stacking` 自动升级；管理员代购固定一周并走叠加规则。玩家购买角色固定为镇长和副镇长，当前没有每商品角色配置键。商品价目、默认等级见 [定价说明](../deployment/PRICING.md)。
