...
> 当前 schema `6.0` 已移除公共资源采购的玩家 GUI、配置目录和管理命令；旧订单表只作为历史数据保留。以下资源订单内容仅用于审计旧版本数据，不再是可用功能。
...
- 购买前 GUI 显示价格、持续时间、叠加规则、下一等级和角色权限。
- 第一次购买、连续升级、价格倍率和最大等级边界与配置一致，每次只有一条扣款流水。
- 双击确认只产生一次购买；余额不足或账户锁定时不创建 `active_buffs`，也不写扣款流水。
- 成员登录、重生、跨世界后效果正确恢复；不允许世界不保留效果，返回允许世界后恢复。
- Attribute Modifier 的稳定 key 为 `tianjitown:buff_<buffKey>`，重复刷新后只有一个 modifier。
- Buff 到期、玩家退镇、被踢或小镇归档后效果完整移除。
- 使用 `/townadmin buff refund <buffId> <原因>` 验证失败补偿：Buff 取消、公共资金退回、退款流水和审计同时存在。
