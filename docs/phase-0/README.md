...
阶段 0 门禁包括四模块 Maven 工程、可重现单 JAR、单元测试、依赖/Vault Economy 可用性检查、异步 SQLite/Flyway 自检，以及带预发双重开关的 Residence API 冒烟测试。早期 YAML 镜像已移除，不参与启动或运行。
...
- 首次启动后确认业务表为空，阶段门禁记录为 1 行，Residence 冒烟领地已完整清理。
- 使用 [`SQLITE_AND_BACKUP.md`](SQLITE_AND_BACKUP.md) 中的方法完成一次在线备份和隔离恢复。
