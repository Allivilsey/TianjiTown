...
final class TownGovernanceUi {
    private final TownUiLegacyFacade facade;

    TownGovernanceUi(TownUiLegacyFacade facade) {
        this.facade = facade;
    }
    ...
    record VoteCreation(UUID townId, VoteType type, UUID subjectId, int memberPage) {
        static VoteCreation parse(String target) {
    ...
    record VoteDecision(UUID voteId, boolean approve) {
        static VoteDecision parse(String target) {
            String[] values = parts(target, 2);
