...
public record ProvisionResult(Status status, ApplicationSnapshot application, MessageRef detail,
    ...
    public static ProvisionResult success(ApplicationSnapshot application) {
        return new ProvisionResult(Status.SUCCESS, application,
                MessageRef.configured(SUCCESS_DETAIL), MessageRef.literal(""));
    }

    public static ProvisionResult failure(ApplicationSnapshot application, String detail,
                                   String recoveryAction) {
        return new ProvisionResult(Status.FAILED, application,
                MessageRef.literal(detail), MessageRef.literal(recoveryAction));
    }

    public static ProvisionResult failure(ApplicationSnapshot application, MessageRef detail,
                                   MessageRef recoveryAction) {
        return new ProvisionResult(Status.FAILED, application,
    ...
    public static ProvisionResult busy(String detail) {
        return new ProvisionResult(Status.BUSY, null, MessageRef.literal(detail),
                MessageRef.configured(BUSY_RECOVERY_ACTION));
    }

    public static ProvisionResult busy(MessageRef detail) {
        return new ProvisionResult(Status.BUSY, null,
                Objects.requireNonNull(detail, "detail"),
                MessageRef.configured(BUSY_RECOVERY_ACTION));
    }

    public static ProvisionResult timeout(ApplicationSnapshot application) {
    ...
    public String detail(PluginMessages messages) {
        return detail.resolve(messages);
    }

    public String recoveryAction(PluginMessages messages) {
        return recoveryAction.resolve(messages);
    }

    public record MessageRef(String messageKey, Map<String, String> placeholders, String literal) {
        public MessageRef {
        ...
        public static MessageRef configured(String messageKey) {
            return configured(messageKey, Map.of());
        }

        public static MessageRef configured(String messageKey, Map<String, ?> placeholders) {
            return new MessageRef(Objects.requireNonNull(messageKey, "messageKey"),
                    stringPlaceholders(placeholders), null);
        }

        public static MessageRef literal(String literal) {
            return new MessageRef(null, Map.of(), Objects.requireNonNull(literal, "literal"));
        }

        public String resolve(PluginMessages messages) {
