...
升级前正常停服并备份 TianjiTown 与依赖数据。公共资金现由小镇数据库独立管理，玩家钱包继续通过 Vault 接入；临时升级逻辑会依据旧配置一次性删除旧版 TianjiTown 绑定 UUID 的 XConomy tax 账户及其余额（保留其他同名账户），不导入旧账户余额。清理条件与 schema 1.1 升级见 [经济部署说明](docs/deployment/ECONOMY_AND_EXPANSION.md)。
