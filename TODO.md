...
- [x] 使用临时 SQLite 文件验证 Flyway 初始化、外键与完整建镇/入镇申请数据链路。
- [ ] 并发批准同一申请时只创建一个小镇。
...
- [ ] 第 1 阶段玩家不使用任何命令完成申请、选址、查看小镇、申请加入和主动退出。
...
- [ ] 启用 `OFFICER` 角色，完成角色权限、入镇申请审核权和 Residence 权限同步。
...
- [ ] 从 `1.0.0` 升级后，已有申请、小镇和 Residence 无需人工重建。
